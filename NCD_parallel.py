...
# ----------------------------
# Multithreaded Assignment to Prototypes
# ----------------------------
def assign_to_prototypes(contents, keys, prototypes, proto_keys, threshold=0.25, max_workers=8):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [executor.submit(assign_single, i) for i in range(len(contents))]
...
def incremental_clustering(folder="rendered_pages_parallel", batch_size=2000, n_initial_prototypes=20, threshold=0.25, force_recompute=False, max_workers=8):
    ...
    for batch_num, batch_files in enumerate(batches):
        ...
        files, contents = load_html_files(folder, batch_files, max_workers=max_workers)
        keys = files
        ...
        if batch_num == 0:
            ...
            assignments, outliers = assign_to_prototypes(contents, keys, prototypes, proto_keys, threshold, max_workers=max_workers)
                ...
                if len(outliers) == 1:
                    ...
                    for idx in new_proto_indices:
                        if 0 <= idx < len(outlier_contents):
                            ...
                            print(f"⚠️ Skipped invalid prototype index: {idx}")
            else:
    ...
    save_cluster_info(full_assignments, file_map, filename="cluster_info.json")
